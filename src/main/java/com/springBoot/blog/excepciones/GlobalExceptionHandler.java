package com.springBoot.blog.excepciones;

import com.springBoot.blog.dto.ErrorDetalles;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<ErrorDetalles> manejarBlogAppException(BlogAppException exception, WebRequest webRequest) {
        ErrorDetalles errorDetalles = new ErrorDetalles(new Date(), exception.getMensaje(), webRequest.getDescription(false));
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        ex.getBindingResult().getAllErrors().forEach(objectError -> {
            String nombreDelCampo = ((FieldError)objectError).getField();
